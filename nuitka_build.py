...
def main(argv=None):
    ...
    try:
        ...
        # Standalone Janitor Sweep (Forensic Parity)
        # We find the .dist or .app and run our specialized prune.py on it.
        # We check both the default Nuitka name ('app') and the target name.
        prune_script = Path(__file__).parent / "scripts" / "prune.py"
        if prune_script.exists():
            for name in ["app", args.name]:
                dist_path = Path(f"{name}.app" if is_mac else f"{name}.dist")
                if dist_path.exists():
                    print(f"\n[JANITOR] Performing Forensic Sweep on {dist_path}...")
                    try:
                        # We use 'sys.executable' to ensure we use the same Python environment
                        subprocess.run(
                            [sys.executable, str(prune_script), str(dist_path)],
                            check=True,
                        )

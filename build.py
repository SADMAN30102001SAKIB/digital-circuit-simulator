...
def main(argv=None):
    ...
    if args.prune and not args.onedir:
        ...
        print(
            "Pruning is a post-processing step that removes files from a directory. It cannot be performed on a single executable."
        )
        sys.exit(2)
    ...
    if args.exclude_qt and not (args.spec_filter or args.onedir):
        print(
            "Tip: --exclude-qt is currently most effective when combined with --spec-filter or --onedir --prune."
        )
    ...
    if args.spec_filter:
        ...
        if pyside_dir:
            ...
            style_plugin = (
                "qwindowsvistastyle" if IS_WIN else ("qmacstyle" if IS_MAC else None)
            )
        ...
        lines.append("")
        lines.append(
            "# Keep ONLY the PySide6 files we explicitly allowed, plus non-PySide6 dependencies"
        )
        ...
        if IS_WIN:
            strict_plugins.extend(
                ["plugins/platforms/qwindows", "plugins/styles/qwindowsvistastyle"]
            )
        elif IS_MAC:
            strict_plugins.extend(
                ["plugins/platforms/qcocoa", "plugins/styles/qmacstyle"]
            )
        elif IS_LINUX:
            strict_plugins.extend(
                ["plugins/platforms/qxcb", "plugins/platforms/libqxcb"]
            )
        ...
        lines.append(
            "    for core in ['qt6core', 'qt6gui', 'qt6widgets', 'qt6dbus', 'qtcore', 'qtgui', 'qtwidgets', 'qtdbus', 'pyside6', 'shiboken6']:"
        )
        ...
        if args.onedir:
            lines.append(
                f"exe = EXE(pyz, a.scripts, [], exclude_binaries=True, name='{args.name}', debug=False, strip={strip_flag}, upx={upx_flag}, console=False, icon=r'{project_root_val}/assets/icon{icon_ext}')"
            )
            lines.append(
                f"coll = COLLECT(exe, a.binaries, a.zipfiles, a.datas, strip={strip_flag}, upx={upx_flag}, name='{args.name}')"
            )
        else:
            lines.append(

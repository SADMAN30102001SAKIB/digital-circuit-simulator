...
def test_truthtable_model_logic(qapp):
    ...
    model = VirtualTruthTableModel(
        [sw1, sw2, and_g], mock_find_source, led=and_g, inputs=[sw1, sw2]
    )
...
def test_truthtable_model_caching(qapp):
    ...
    model = VirtualTruthTableModel(
        [sw1, and_g], mock_find_source, led=and_g, inputs=[sw1]
    )
...
def test_truthtable_model_large(qapp):
    ...
    model.data(model.index(1000000, 0), Qt.DisplayRole)
    assert 1000000 in model._cache
